...
class SampleBotAi(BotAi):
    ...
    def make_move(self, bot_id: int, game_state: GameState) -> Move:
        ...
        if self._is_dead():
            return DEFAULT_MOVE
        for move_supplier in (self._close_treasure_move_supplier, self._close_battery_move_supplier,
                              self._reachable_treasure_move_supplier, self._reachable_battery_move_supplier,
